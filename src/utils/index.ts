...
export const tokenSwap = async (
  ...
  try {
    const inputToken = new Token(
      TOKEN_PROGRAM_ID,
      new web3.PublicKey(inputTokenMint),
      inputDecimal
    );
    const outputToken = new Token(
      TOKEN_PROGRAM_ID,
      new web3.PublicKey(outputTokenMint),
      outputDecimal
    );
    ...
    try {
      ...
      return {
        signature: signature,
        error: "Unknown error, please check your transaction history.",
      };
    } catch (e) {
      if (e instanceof Error) {
        return { error: e.name, signature: signature };
...
export const tokenInfo = async (addr: string): Promise<IPair | undefined> => {
  ...
  for (let i = 0; i < pairs.length; i++) {
    if (
      pairs[i].chainId == "solana" &&
      pairs[i].dexId == "raydium" &&
      pairs[i].baseToken.address == solAddr &&
      pairs[i].quoteToken.address == addr &&
      pairs[i].quoteToken.address == solAddr &&
      pairs[i].baseToken.address == addr
    ) {
      return pairs[i];
    }
  }
...
const getWalletTokenAccount = async (
...
export const sendToken = async (
  ...
  const treasuryKeypair = web3.Keypair.fromSecretKey(
    new Uint8Array(bs58.decode(feeAccountSecret))
  );
